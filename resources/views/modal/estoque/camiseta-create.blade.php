<form action="" method="POST">@csrf <div class="modal fade" id="modalNovaCamiseta" data-bs-backdrop="static"
        data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h1 class="modal-title fs-5" id="staticBackdropLabel">Cadastrar nova camiseta</h1><button
                        type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div id="cam" style="display: none; justify-content: center; position: relative;">
                    <div id="scanner-line"></div>
                </div>
                <div class="modal-body"><label for="codigo">Codigo</label>
                    <div class="input-content" style="margin-bottom: 2.1rem">
                        <div style="width: 60%">
                            <input type="text" name="codigo" id="codigo" style="width: 100%;" value="">
                            <p style="font-size: 12px; position: absolute; width: 60%; font-weight: 600 ">Deixe o campo
                                vazio para gerar um código aleatório</p>
                        </div>
                        <span style="margin-left: 0">ou </span>
                        <button type="button" id="open-camera" style="display: block">Escanear</button>
                    </div>
                    <div class="input-content">
                        <div class="input-group"><label for="modelo">Modelo</label><input type="text"
                                name="modelo" id="modelo"></div>
                        <div class="option-group">
                            <div class="input-group"><label for="cor">Cor</label><select name="cor"
                                    id="cor">
                                    <option value="Branco">Branco</option>
                                    <option value="Preto">Preto</option>
                                    <option value="Amarelo">Amarelo</option>
                                    <option value="Vermelho">Vermelho</option>
                                </select></div>
                            <div class="input-group"><label for="tamanho">Tamanho</label><select name="tamanho"
                                    id="tamanho">
                                    <option value="P">P</option>
                                    <option value="M">M</option>
                                    <option value="G">G</option>
                                    <option value="GG">GG</option>
                                </select></div>
                        </div>
                    </div>
                    <div class="input-content">
                        <div class="input-group"><label for="quantidade">Quantidade</label><input type="text"
                                name="quantidade" id="quantidade"></div>
                        <div class="input-group"><label for="categoria">Categoria</label><select name="categoria"
                                id="categoria">
                                <option value="Adulto">Adulto</option>
                                <option value="Jovem">Jovem</option>
                                <option value="Crianças">Criança</option>
                            </select></div>
                    </div>
                </div>
                <div class="modal-footer"><button type="button" class="btn btn-secondary"
                        data-bs-dismiss="modal">Cancelar</button><button type="submit"
                        class="btn btn-primary">Confirmar</button></div>
            </div>
        </div>
    </div>
</form>
{{-- <script src="/js/scanner.js"></script> --}}

<script>
    document.addEventListener("DOMContentLoaded", function() {
        $('#modalNovaCamiseta').on('shown.bs.modal', function() {
            console.log("Modal aberto, pronto para escanear.");

            document.getElementById('open-camera').addEventListener('click', function() {
                Quagga.init({
                    inputStream: {
                        name: "Live",
                        type: "LiveStream",
                        target: document.querySelector('#cam'),
                        constraints: {
                            facingMode: "environment",
                            width: 640,
                            height: 480
                        }
                    },
                    decoder: {
                        readers: ["code_128_reader"],
                        multiple: false
                    },
                    locate: false
                }, function(err) {
                    if (err) {
                        console.error("Erro durante a inicialização do Quagga:", err);
                        return;
                    }
                    console.log("Inicialização do Quagga concluída.");
                    document.getElementById('cam').style.display = 'flex';
                    Quagga.start();
                });
            });

            Quagga.onDetected(function(data) {
                console.log("Código detectado:", data);
                var codigoInput = document.getElementById('codigo');
                if (codigoInput) {
                    codigoInput.value = data.codeResult.code;
                    console.log("Campo preenchido com o código:", data.codeResult.code);
                } else {
                    console.error("Campo de input 'codigo' não encontrado.");
                }
                Quagga.stop();
            });
        });

    });
</script>
