<x-ui.action-section>
    ...
    <x-slot name="content">
        <div class="max-w-xl text-sm description">
            ...
            <div class="mt-5 space-y-6">
                ...
                @foreach ($this->sessions as $session)
                    <div class="flex items-center">
                        <div>
                            @if ($session->agent->isDesktop())
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                    stroke-width="1.5" stroke="currentColor" class="w-8 h-8 session-icon">
                        ...
                        <div class="ms-3">
                            <div class="text-sm session-plataform">
                                {{ $session->agent->platform() ? $session->agent->platform() : __('Unknown') }} -
                                {{ $session->agent->browser() ? $session->agent->browser() : __('Unknown') }}
                            </div>

                            <div>
                                <div class="text-xs session-ip">
                                    {{ $session->ip_address }},

                                    @if ($session->is_current_device)
                                        <span class="session-device font-semibold">{{ __('This device') }}</span>
                                    @else
        ...
        <div class="flex items-center mt-5">
            ...
            <x-ui.action-message class="ms-3" on="loggedOut">
                {{ __('Done.') }}
            </x-ui.action-message>
        </div>

        <!-- Log Out Other Devices Confirmation Modal -->
        <x-ui.modal wire:model.live="confirmingLogout" :id="'logoutModal'">
            <x-slot name="title">
                {{ __('Log Out Other Browser Sessions') }}
            </x-slot>
            ...
            <x-slot name="content">
                ...
                <div class="mt-4" x-data=""
                    ...
                    <x-ui.input type="password" class="mt-1 form-control" autocomplete="current-password"
